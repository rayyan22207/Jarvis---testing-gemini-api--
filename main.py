...
# main.py
from helpers import print_section
from kb_qa import answer_from_kb

def main():
    print_section("KB-QA TEST")
    q = input("Ask about the KB (e.g., 'How does WebRTC connect peers?'): ").strip()
    if not q:
        print("No question given.")
        return
    ans = answer_from_kb(q, json_path="data/sample.json")
    print("\nAnswer:\n" + ans)
